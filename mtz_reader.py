...
def plot (res_lst, ampli_lst, pdb_id, max_res, marker):
    ...
    """
    auspex_rng = np.genfromtxt("Helcaraxe_program/auspex_ranges.csv", delimiter=';')
    ...
    for i, element in enumerate(auspex_rng):
        ...
        if max_res < res_bin_end:
            ...
            bin_arr = ndimage.rotate(bin_arr, 90) #I Dont know why I have to do that, but it works
...
def master (plot_list, class_list):
    ...
    master_array.to_pickle(root+"arrays/paper_{}.pkl".format(version))
    master_array.to_excel(root+"files/paper_{}.xlsx".format(version))
    np.save(root+"arrays/paper__binplots_{}.npy".format(version),plot_list)

def main():
    global plot_list, id_list, root, class_list
    root = "/Users/kristophernolte/Documents/GitHub/helcaraxe/"
    class_list = pd.read_pickle(root + "arrays/cleaned_test.pkl".format(version))
    plot_list, id_list, stat_list_l, stat_list_r, missing_keys = [], [], [], [], []

    for i,key in enumerate(class_list["PDB-ID"]):
        key = key[:4]
        try:
            mtz = gemmi.read_mtz_file("/Users/kristophernolte/Documents/GitHub/helcaraxe/mtz/"+key+".mtz")
            mtz_reader(mtz, key)
