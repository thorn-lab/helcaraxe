...
def plot (res_lst, ampli_lst, pdb_id, max_res, marker):
    ...
    for i, element in enumerate(auspex_rng):
        ...
        if max_res < res_bin_end:
            ...
            bin_arr, xedges, yedges = np.histogram2d(res_lst, ampli_lst, range=image_bin, bins=60)
...
def master (plot_list):
    ...
    master_array.to_pickle(root+"arrays/master_array_{}.pkl".format(version))
    master_array.to_excel(root+"files/master_array_{}.xlsx".format(version))
    np.save(root+"arrays/master_binplots_{}.npy".format(version),plot_list)
...
def main():
    ...
    for i,key in enumerate(class_lst["PDB-ID"]):
        key = key[:4]
        try:
            mtz = gemmi.read_mtz_file("/Users/kristophernolte/Documents/AG_Thorn/mtz/"+key+".mtz")
            mtz_reader(mtz, key)
