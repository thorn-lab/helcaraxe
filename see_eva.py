...
plots_F = np.load("arrays/master_binplots_test_Iobs_cleaned.npy")
master_F = pd.read_pickle("arrays/master_array_test_Iobs_cleaned.pkl")
...
def show_plot(plot, i):
    ...
    plt.savefig("{}.svg".format(str(i)+"_"+name_lst[i]+"_"+str(classification[i])))
    plt.show()
...
for element in wrong:
    j = element
    show_plot(plots_F[int(element)],int(element))
